...
ModelDrawer::ModelDrawer()
    : shader_{new s21::Shader("shaders/versh.glsl", "shaders/fragm.glsl")} {
...
void ModelDrawer::Draw(std::size_t size, const Vertex3D* start, GLuint type) {
  glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer_);  // lishnee na vsiakiy

  glBufferData(GL_ARRAY_BUFFER, size * sizeof(Vertex3D), start, GL_STATIC_DRAW);
...
void ModelDrawer::MakeMVP(bool ortho) {
  ...
  if (ortho)
    ...
    projection =
        glm::perspective(glm::radians(45.0f), 1280.0f / 750.0f, 0.1f, 100.0f);
